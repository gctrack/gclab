...
const styles = `
  ...
  .gbtn-sm:hover { background: #86efac; transform: translateY(-1px); box-shadow: 0 6px 18px rgba(74,222,128,0.28); }
...
export default function HomePage() {
  ...
  return (
    <>
      ...
      {/* NAV */}
      <nav style={{ position:'sticky', top:0, zIndex:100, display:'flex', alignItems:'center', justifyContent:'space-between', padding:'14px 48px', background:green, borderBottom:'1px solid rgba(255,255,255,0.07)' }}>
        <Link href="/" style={{ display:'flex', alignItems:'center', gap:11, textDecoration:'none' }}>
          <Logo/>
          <span className="ghl" style={{ fontSize:22, color:cream, fontWeight:700 }}>GCLab</span>
        </Link>
        <div className="g-nav-links" style={{ display:'flex', alignItems:'center', gap:28 }}>
          <Link href="/rankings" style={{ color:'rgba(232,224,208,0.5)', textDecoration:'none', fontSize:14 }}>Rankings</Link>
          <Link href="/compare"  style={{ color:'rgba(232,224,208,0.5)', textDecoration:'none', fontSize:14 }}>Compare</Link>
          <Link href="/rankings?tab=historical" style={{ color:'rgba(232,224,208,0.5)', textDecoration:'none', fontSize:14 }}>Historical</Link>
          <Link href="/login" className="gbtn-sm">Sign In / Sign Up</Link>
        </div>
      </nav>
      ...
      <section className="g-hero g-section" style={{ background:green, minHeight:'86vh', display:'grid', gridTemplateColumns:'1fr 1fr', alignItems:'center', padding:'80px 48px', gap:40, position:'relative', overflow:'hidden' }}>
        ...
        <div style={{ position:'relative', zIndex:1 }}>
          ...
          <div style={{ display:'flex', alignItems:'center', gap:18, flexWrap:'wrap' }}>
            ...
            <Link href="/login" className="gbtn-ghost">Create free account →</Link>
          </div>
        </div>
      ...
      <section className="g-section" style={{ background:green, padding:'80px 48px' }}>
        <div style={{ maxWidth:960, margin:'0 auto' }}>
            ...
            <div style={{ padding:'24px 28px 12px' }}>
              <svg width="100%" height="240" viewBox="0 0 880 240" preserveAspectRatio="none">
                ...
                {([[265,106],[460,60],[645,134]] as [number,number][]).map(([x,y])=>(
                  <g key={x}>
                    <polygon points={`${x},${y} ${x-4},${y+8} ${x+4},${y+8}`} fill="#eab308" opacity="0.8"/>
                    <text x={x} y={y+19} fill="rgba(234,179,8,0.65)" fontSize="9" fontFamily="DM Sans,sans-serif" textAnchor="middle">WC</text>
                  </g>
                ))}
              </svg>
              <div style={{ display:'flex', gap:20, marginTop:10, flexWrap:'wrap' }}>
                {[{l:'dGrade',c:'#4ade80',dot:false,dash:false},{l:'eGrade',c:'#60a5fa',dot:false,dash:true},{l:'WCF Event',c:'#eab308',dot:true,dash:false},{l:'Peak grade',c:'#4ade80',dot:true,dash:false}].map(({l,c,dot,dash})=>(
                  <div key={l} style={{ display:'flex', alignItems:'center', gap:6, fontSize:12, color:'rgba(232,224,208,0.45)' }}>
                    {dot ? <span style={{ width:8, height:8, borderRadius:'50%', background:c, display:'inline-block' }}/> : <span style={{ display:'inline-block', width:20, height:2, background:dash?'none':c, borderTop:dash?`2px dashed ${c}`:'none', opacity:dash?0.6:1 }}/>}
                    {l}
                  </div>
                ))}
              </div>
          ...
          <div style={{ marginTop:22, display:'flex', alignItems:'center', gap:20, flexWrap:'wrap' }}>
            <Link href="/login" className="gbtn">See Your Grade History →</Link>
      ...
      <section style={{ background:'linear-gradient(135deg,#0a2014 0%,#1a4030 50%,#0a2014 100%)', padding:'100px 48px', textAlign:'center', position:'relative', overflow:'hidden' }}>
        ...
        <div style={{ position:'relative', maxWidth:560, margin:'0 auto' }}>
          ...
          <Link href="/login" className="gbtn" style={{ fontSize:16, padding:'15px 32px' }}>Create Free Account →</Link>
