  ...
  useEffect(() => {
    ...
    const buildOppStats = async (games: Game[], setter: (v: any[]) => void) => {
      ...
      setter(Object.entries(countryMap)
        .map(([country, s]) => ({ country, games: s.games, wins: s.wins, losses: s.games - s.wins, winPct: s.games ? Math.round(s.wins / s.games * 100) : null }))
        ...
        {(playerA || playerB) && (
          <div className="space-y-6">
                  ...
                  <table className="w-full text-sm">
                    <thead>
                      <tr className="border-b border-gray-100 bg-gray-50">
                        ...
                        <th className="text-right text-xs text-green-600 font-medium py-2.5 px-2">{nameA.split(' ')[0]}</th>
                        <th className="text-right text-xs text-gray-400 font-medium py-2.5 px-2">G</th>
                        <th className="text-right text-xs text-blue-600 font-medium py-2.5 px-2">{nameB.split(' ')[0]}</th>
                        <th className="text-right text-xs text-gray-400 font-medium py-2.5 px-4">G</th>
                      </tr>
                    </thead>
                    <tbody>
            ...
            {(countryStatsA.length > 0 || countryStatsB.length > 0) && (
              <section>
                <h3 className="text-sm font-semibold text-gray-500 uppercase tracking-wide mb-3">Results by Country Played In</h3>
                <div className="bg-white rounded-xl border border-gray-100 shadow-sm overflow-hidden">
                  <table className="w-full text-sm">
                    <thead>
                      <tr className="border-b border-gray-100 bg-gray-50">
